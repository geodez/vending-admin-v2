...
export default function OwnerReportTab() {
  ...
  if (state.kind === 'forbidden' || state.kind === 'error') {
    return (
      <Alert
        message={state.kind === 'forbidden' ? 'Доступ запрещен' : 'Ошибка загрузки'}
        description={state.kind === 'forbidden' ? 'Отчёт собственника доступен только роли owner.' : (state.message || 'Не удалось получить данные отчёта.')}
        type="error"
        showIcon
      />
    );
